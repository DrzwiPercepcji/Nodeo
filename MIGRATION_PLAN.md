...
- [x] **CI/CD (GitHub Actions) — baseline** — `.github/workflows/ci.yml` runs on every PR and push to `master`/`main`: backend `lint` + `typecheck` + `test` (Vitest), frontend `lint` + `type-check` + `test` (Vitest + Vue Test Utils) + `build-only` (Node 24).
- [x] **CI/CD — extend** — E2E: Playwright + Cucumber (Gherkin, English scenarios), job in `.github/workflows/ci.yml` brings up `docker compose` with `docker-compose.yml` + `docker-compose.local.yml` + `docker-compose.e2e.yml`, `e2e/fixtures` (optional `sample.mp3` / `sample.mp4`; CI generates tiny files via ffmpeg if missing). See `docs/e2e.md`, `docs/docker.md`.
...
- [x] **Frontend unit / E2E (GHA)** — Vitest + happy-dom + `@vue/test-utils` + `@pinia/testing`: `frontend/src/stores/__tests__/auth.spec.ts` (mocked API client), `frontend/src/views/__tests__/LoginView.spec.ts` (PrimeVue + router + Pinia). Run `npm run test` in `frontend/`; same workflow as E2E above. Coverage thresholds not configured (optional).
